...
function setGroup(id) {
  if (currGroup) {
    ...
    document.querySelector(`.toggle-${currGroup}-active`).classList.add('hidden')
    document.querySelectorAll(`.group-${currGroup}`).forEach(
  ...
  if (currGroup) {
    ...
    document.querySelector(`.toggle-${currGroup}-active`).classList.remove('hidden')
    document.querySelectorAll(`.group-${currGroup}`).forEach(
...
function setQuestion(id) {
  if (currQuestion === id) {
    document.querySelectorAll(`.open-${currQuestion}`).forEach(
  ...
  if (currQuestion) {
    ...
    document.querySelector(`.open-toggle.open-${currQuestion}`).classList.remove('hidden')
  }
  currQuestion = id
  if (currQuestion) {
    document.querySelectorAll(`.open-${currQuestion}`).forEach(
    ...
    document.querySelector(`.open-toggle.open-${currQuestion}`).classList.add('hidden')
  }
}
...
function resetQuery() {
  ...
  document.querySelectorAll('#main > div').forEach(
    e => e.classList.add('hidden')
  )
  setGroup(currGroup)
}
...
function handleInput(e) {
  ...
  questions.forEach((i, j) => {
    if (checkString(i, e.target.value)) {
      document.querySelector('.item-' + j.toString().padStart(3, '0')).classList.remove('hidden')
    } else {
      document.querySelector('.item-' + j.toString().padStart(3, '0')).classList.add('hidden')
    }
  })
